...
class Isolation(SupportComponentBase if BASE_AVAILABLE else object):
    ...
    def is_empty(self) -> bool:
        ...
        return not self.w or all(w == 0 for w in self.w) or not self.h or all(h == 0 for h in self.h)
...
def create_kapton_isolation(r0: float, thickness: float, height: float, 
                           n_layers: int = 1) -> Isolation:
    """
...
def create_graded_isolation(r0: float, thickness_start: float, thickness_end: float, 
                           height: float, n_layers: int = 3) -> Isolation:
    """
