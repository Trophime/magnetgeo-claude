...
class Supra(MagnetComponentBase if BASE_CLASS_AVAILABLE else yaml.YAMLObject):
    ...
    def check_dimensions(self, magnet: Any = None):
        ...
        if changed:
            print(f"Supra/check_dimensions: Updated dimensions for {self.name} from {self.struct}")
            print(self)
    ...
    def get_insulator_info(self) -> tuple:
        ...
        if magnet and hasattr(magnet, 'get_names'):
            ...
            insulation_count = sum(1 for name in names if 'insul' in name.lower() or 'mandrin' in name.lower())
        ...
        if self.detail == "None":
            ...
            if magnet:
                ...
                if hasattr(magnet, 'get_names'):
                    return magnet.get_names(mname=mname, detail=self.detail, verbose=verbose)
    ...
    def get_structure_info(self) -> dict:
        ...
        if magnet:
            try:
                info.update({
                    "structure_r0": magnet.getR0() if hasattr(magnet, 'getR0') else None,
                    "structure_r1": magnet.getR1() if hasattr(magnet, 'getR1') else None,
                    "structure_height": magnet.getH() if hasattr(magnet, 'getH') else None,
                    "structure_turns": sum(magnet.getNtapes()) if hasattr(magnet, 'getNtapes') else None,
                    "dblpancake_count": magnet.getN() if hasattr(magnet, 'getN') else None,
                })
            except Exception as e:
