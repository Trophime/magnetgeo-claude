...
try:
    ...
    try:
        from ..support.chamfer import Chamfer
        from ..support.groove import Groove
        from ..shape import Shape
        from ..support.modelaxi import ModelAxi
        from ..support.model3d import Model3D
...
class Helix(MagnetComponentBase if BASE_CLASS_AVAILABLE else yaml.YAMLObject):
    ...
    def chamfers(self) -> List[Any]:
        ...
        for i, chamfer_data in enumerate(self._chamfers_data):
            try:
                chamfer = self._load_support_object(
                    chamfer_data, Chamfer, f"Chamfer[{i}]"
                )
                if chamfer:
    ...
    def _load_support_object(
        self, data: Any, expected_class: type, context: str
    ) -> Optional[Any]:
        """
            ...
            try:
                with open(f"{data}.yaml", "r") as f:
                    ...
                    if isinstance(yaml_data, expected_class):
                        return yaml_data
                    elif hasattr(expected_class, "from_dict") and isinstance(
                        yaml_data, dict
                    ):
    ...
    def get_type(self) -> str:
        ...
        if hasattr(self.model3d, "get_model_type"):
            ...
            self.model3d
            and getattr(self.model3d, "with_shapes", False)
            and getattr(self.model3d, "with_channels", False)
        ):
            return "HR"
        return "HL"
    ...
    def generate_cut(self, format: str = "SALOME"):
        ...
        try:
            ...
            if self.model3d and getattr(self.model3d, "with_shapes", False):
                ...
                if self.shape:
                    angles_str = " ".join(
                        f"{t:4.2f}" for t in getattr(self.shape, "angle", []) if t != 0
                    )
